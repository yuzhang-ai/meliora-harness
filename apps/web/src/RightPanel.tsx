...
export function RightPanel(props: RightPanelProps) {
  ...
  return <>
    <div className="right-tabs" role="tablist" aria-label="右侧面板">
      ...
      <span className="right-tools"><Icon name="code"/><Icon name="external"/><Icon name="panel"/></span>
    </div>
    {activeTab === "changes" ? <div className="changes-empty" role="status"><span aria-hidden="true"><Icon name="code" size={22}/></span><h3>暂无文件变更</h3><p>当前 fixture 只回放公开运行事件，尚未提供 Diff。</p><small>产生真实变更后，这里将显示文件与行级差异。</small></div> : <div className="preview-empty"><span><Icon name="panel" size={22}/></span><h3>浏览器预览</h3><p>{props.projectName} / {props.chatTitle}</p><small>连接本地预览服务后在这里显示页面。</small></div>}
    {props.noticeVisible && <aside className="privacy-notice" role="status">
      <button className="notice-close" aria-label="关闭通知" onClick={props.onCloseNotice}><Icon name="x" size={14}/></button>
      <h3>分析与 Cookie</h3>
