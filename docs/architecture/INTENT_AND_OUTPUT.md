...
该 v1 的目的，是让同一用户看到经只读工具验证后的最终回答；它不是 raw streaming，也不把模型文本升级为 Receipt、verification 或完成事实。首次/中间 assistant text、用户输入回显、private reasoning 与未验证的纯文本回答继续只写 private event/history。Runtime 必须把用户输入、此前 private reasoning、中间 assistant text，以及 Tool projector 明确标记的 `privateObservationContent` 一并作为 `privateFinalAnswerObservations` 交给最终 projector；projector 对其原文、紧凑形式、percent encoding、Base64 与 Base64url 回显 fail closed。Tool projector 只有在工具成功、verification=`passed` 且能从私有 artifact 推导出固定、无原文内容的 Server-owned fact 时，才可把 `privateObservationContent` 设为 `null`；当前唯一特例要求 `git status --porcelain` artifact 同时满足零字节 content、`toolName=git_status`、`encoding=utf-8`、`bytesRead=0` 与 `truncated=false`，然后投影为“Git 工作区干净，没有未提交修改”。binary、截断、非零空白、任何非空 Git 输出及其他原始工具 observation 都不能使用该特例。公网部署还必须只绑定已经确认可公开的固定工作区；projector 不能替代工作区内容治理。
